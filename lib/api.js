...
export async function apiCall(action, data = {}) {
    ...
    try {
        ...
        try {
            ...
            if (!parsed.success) {
                if (typeof window !== 'undefined' && window.appLog) window.appLog('error', `[API FAIL] ${action}: `, parsed.error || parsed);
                console.error(`[API FAIL] ${action}: `, parsed.error || parsed);
            } else {
                if (typeof window !== 'undefined' && window.appLog) window.appLog('log', `[API OK] ${action}: `, parsed);
                console.log(`[API OK] ${action}: `, parsed);
            }
            return parsed;
        } catch {
            if (typeof window !== 'undefined' && window.appLog) window.appLog('error', `[API CRASH] ${action} - Invalid Response: `, text);
            console.error(`[API CRASH] ${action} - Invalid Response: `, text);
            return { success: false, error: 'Server error — invalid response' };
        }
    } catch (err) {
        if (typeof window !== 'undefined' && window.appLog) window.appLog('error', 'API call failed:', err);
...
export function hasRole(role) {
    ...
    const roles = (user.roles || user.role || '').split(',').map(r => {
...
export function getUserRoles() {
    const user = getUser();
    if (!user) return false;
    return (user.roles || user.role || '').split(',').map(r => {
        let normalized = r.trim().toLowerCase().replace(/\s+/g, '_');
        if (normalized === 'site_admin') return 'admin';
        return normalized;
    }).filter(Boolean);
...
export const updateProfile = (updates) =>
    apiCall('updateProfile', typeof updates === 'string' ? { avatar_base64: updates } : updates);
...
// Design & Files
export const uploadFile = (job_id, filename, mimeType, base64Data) =>
    apiCall('uploadFile', { job_id, filename, mimeType, base64Data });

export const sendDesignReview = (job_id, filename, mimeType, base64Data, messageToClient) =>
    apiCall('sendDesignReview', { job_id, filename, mimeType, base64Data, messageToClient });

export const getJobPublic = (job_id) =>
    apiCall('getJobPublic', { job_id });

export const submitDesignFeedback = (job_id, approved, feedback) =>
    apiCall('submitDesignFeedback', { job_id, approved, feedback });
...
// Products
export const getProducts = (status = null) => apiCall('getProducts', { status });
export const addProduct = (payload) => apiCall('addProduct', payload);
export const updateProduct = (item_id, updates) => apiCall('updateProduct', { item_id, ...updates });
export const deleteProduct = (item_id) => apiCall('deleteProduct', { item_id });

// Expenses
export const getExpenses = (category = 'all') => apiCall('getExpenses', { category });
export const addExpense = (payload) => apiCall('addExpense', payload);
export const updateExpense = (expense_id, updates, edit_memo) => apiCall('updateExpense', { expense_id, ...updates, edit_memo });
export const deleteExpense = (expense_id, edit_memo) => apiCall('deleteExpense', { expense_id, edit_memo });
